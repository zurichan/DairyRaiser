...
if (isset($_GET['bi_code']) && isset($_GET['page'])) {
    ...
    $invoice_row = 0;
    $sold_buffalo_row = $api->Read('bi_session', 'set', 'is_sold', '1', true);
    ...
    if ($ptoken == 2 || $page == 'create' || $page == 'sold_buffalo' || $page == 'return' || $page == 'all') {
        ...
        if ($page == 'edit') {
            ...
            $header = "View Invoice Record | Dairy Raisers";
            $title_page = "View Invoice Record";

            $amountpcs = '';
            ($fetch_session_row > 1) ? $amountpcs = 'pcs' : $amountpcs = 'pc';
        } else if ($page == 'create') {
            ...
            $fetch_sold_buffalos = $api->Read('bi_session', 'set', 'is_sold', '1');
        } else if ($page == 'return') {
...
?>
        ...
        <div class="border-bottom d-flex flex-row justify-content-between align-items-center overflow-hidden pb-2 mb-3">
            <div class="d-flex justify-content-center align-items-center flex-row">
                ...
                <div class="d-flex flex-column justify-content-center align-items-start me-4">
                    ...
                    <div class="nav-item d-flex flex-row justify-content-center align-items-center">
                        ...
                        <a href="./invoice_list.php?page=all&bi_code=none" class="btn btn-sm btn-outline-primary me-2 <?php if (isset($_GET['page']) && $page == 'all') echo 'active'; ?>">All</a>
                        <a href="./invoice_list.php?page=sold_buffalo&bi_code=none" class="btn btn-sm btn-outline-primary <?php if (isset($_GET['page']) && $page == 'sold_buffalo') echo 'active'; ?> me-2">Sold Buffalo</a>
                        <a href="./invoice_list.php?page=return&bi_code=none" class="btn btn-sm btn-outline-danger <?php if (isset($_GET['page']) && $page == 'return') echo 'active'; ?> me-2">Return</a>
                    </div>
                </div>

                <div class=" d-flex flex-column justify-content-center align-items-center py-2 px-4 rounded bg-primary text-light">
                    <h1 class="lead py-0 mb-3" style="font-size: 27px;">Total Sales <i class="bi bi-check2-circle"></i> <span class="ms-2">:</span> <span class="fw-bold ms-2">₱<?= $total_sales; ?>.00</span></h1>
                    <div class="d-flex flex-row justify-content-between align-items-center text-light">
                        <h1 class="lead py-0 opacity-75 text-center d-flex justify-content-between align-items-center  me-4" style="font-size: 12px;">Invoice <i class="bi bi-receipt mx-1"></i> : <span class="ms-1 fw-bold"><?= $invoice_row; ?></span></h1>
                        <h1 class="lead py-0 opacity-75 text-center d-flex justify-content-between align-items-center  me-4" style="font-size: 12px;">Sold Buffalo <i class="bi bi-coin mx-1"></i> : <span class="ms-1 fw-bold"><?= $sold_buffalo_row; ?></span></h1>
                        <h1 class="lead py-0 opacity-75 text-center d-flex justify-content-between align-items-center  me-4" style="font-size: 12px;">Return <i class="bi bi-box-arrow-in-left mx-1"></i>: <span class="ms-1 fw-bold"><?= $return_bi_row; ?></span></h1>
                    </div>
                </div>
            </div>
            <?php if (isset($_GET['page']) && $page == 'all') { ?>
                <div class="modal fade" id="CreateInvoice" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                        <form class="modal-content" method="POST" action="../process/process-invoice.php" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="w-100 d-flex justify-content-center align-items-stretch">
                                        <div class="form-check me-3">
                                            <input class="form-check-input" type="radio" name="selectInvoiceCategory" id="buffaloInvoice" checked>
                                            <label class="form-check-label" for="buffaloInvoice">
                                                Buffalo
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="selectInvoiceCategory" id="milkInvoice">
                ...
                <div class="">
                    <button type="button" class="btn btn-sm btn-success" data-bs-target="#CreateInvoice" data-bs-toggle="modal">Create New Invoice</button>
            ...
            <?php } else if (isset($_GET['page']) && $_GET['page'] == 'view') { ?>
                <div class="">
                    <button type="button" onclick="doPrint()" class="btn btn-primary me-2"><i class="bi bi-printer"></i> Print</button>
    ...
    <!-- MAIN CONTENT -->
    <?php if (isset($_GET['page']) && $page == 'edit') { ?>
        <div class="d-flex flex-row justify-content-between align-items-center mt-4">
            <div class="p-3 border-bottom d-flex flex-column justify-content-center align-items-center" style="width: 100%;">
                <form method="POST" action="../process/process-buffalo.php" class="mb-4 d-flex flex-row justify-content-between align-items-center w-100">
                    ...
                    <div class="d-flex flex-row justify-content-start align-items-center w-100">
                        <div class="form-floating me-2 w-100">
                            <select name="selected_buffalo" id="selected_buffalo" class="form-select">
                                ...
                                foreach ($buffalos as $selected_buffalo) {
                                    if ($selected_buffalo->Marked_As != 'Deceased') {
                        ...
                        <button type="button" name="buffalo_invoice_submit" id="buffalo_invoice_submit" class="btn btn-primary">Add</button>
                    </div>
                </form>
                ...
                <form class="card-body" method="POST" action="../process/process_invoice.php">
                    <div class="d-flex flex-row justify-content-center align-items-center">
                        <div class="form-floating mb-3 w-100">
                            <textarea class="form-control" placeholder="Leave a remarks here" style="height: 100px;" name="Remarks" id="floatingTextarea"><?= $remarks; ?></textarea>
                            <label for="floatingTextarea">Remarks</label>
                        </div>
                    </div>
                    <div class="form-floating mb-4 w-100">
                        <input type="text" class="form-control" placeholder="buyer name" value="<?= $client; ?>" name="client_name" id="client_name">
                        <label for="client_name">Client Name:</label>
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text" id="basic-addon1">₱</span>
                        <div class="form-floating">
                            <input type="number" class="form-control" value="<?= $otherFees; ?>" placeholder="other fees" name="other_fees" id="other_fees">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <h6 class="card-subtitle mb-2 text-muted text-center d-flex justify-content-center align-items-center">
                            Discount:
                            <input style="width: 45px;" type="number" aria-describedby="discount" value="<?= $discount; ?>" class="form-control mx-2" oninput="maxlength()" name="discount" id="discount" placeholder="0">
                            %
                        </h6>
                    </div>
                    ...
                    <a href="./invoice_list.php" class="card-link btn btn-secondary">Back</a>
                </form>
            </div>
        </div>
        <?php } else if (isset($_GET['page']) && $page == 'view') {
        ...
        ?>
            <div id="print-container">
                <div class="px-2">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <p class="lead" style="font-size: 15px;"><span class="fw-bold">Customer Name: </span><?= $fetch_bi[0]->client; ?></p>
        ...
        ?>
    <?php } else if (isset($_GET['page']) && $page == 'create') { ?>
        <div class="d-flex flex-row justify-content-between align-items-center mt-4">
            <div class="p-3 border-bottom d-flex flex-column justify-content-center align-items-center" style="width: 100%;">
                <form method="POST" action="../process/process-buffalo.php" class="mb-4 d-flex flex-row justify-content-between align-items-center w-100">
                    <h1 class="lead text-start">Input Form</h1>
            ...
            <div class="card" style="width: 100%;">
                <form class="card-body" method="POST" action="../process/process_invoice.php">
                    <div class="d-flex flex-row justify-content-center align-items-center">
                        <div class="form-floating mb-3 w-100">
                            <textarea class="form-control" placeholder="Leave a remarks here" style="height: 100px;" name="Remarks" id="floatingTextarea"></textarea>
                            <label for="floatingTextarea">Remarks</label>
                        </div>
                    </div>
                    <div class="form-floating mb-3 w-100">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <h6 class="card-subtitle mb-2 text-muted text-center d-flex justify-content-center align-items-center">
                            Discount:
                            <input style="width: 45px;" type="number" aria-describedby="discount" class="form-control mx-2" oninput="maxlength()" name="discount" id="discount" placeholder="0">
                            %
                        </h6>
                    </div>
                    ...
                    <button type="submit" name="create_bi_invoice" class="card-link btn btn-primary">Submit</button>
                    <a href="./invoice_list.php" class="card-link btn btn-secondary">Back</a>
                </form>
            </div>
        </div>
    ...
    <?php } else if (isset($_GET['page']) && $page == 'all') { ?>
        <div class="table-responsive">
            <table class="table table-hover table-bordered table-striped" id="buffaloTable" style="width: 100%; font-size: 14px;">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Date</th>
    ...
    <script>
        function doPrint() {
            $(document).ready(() => {
                ...
                document.querySelector('#body').innerHTML = data + footer;
                setTimeout(() => {
                    window.print();
    ...
    <script>
        ...
        function RetrieveBtns() {
            retrieve_btn.forEach((data, i) => {
                $(data).click(() => {
                    swal({
                        title: "Retrieve Invoice: " + bi_code[i].innerText + " ?",
                        closeOnClickOutside: false,
                        buttons: true,
                    }).then((willDelete) => {
                        if (willDelete) {
